...
module.exports = class ServerAIS {
	...
	async synchronize() {
		...
		try {
			...
			// Debug Helper - process.stdout.write(placemarks.length.toString());
			...
			// Debug Helper - console.log('\t - last MarineTraffic time: ' + lastMTUpdate);
			...
			this.disconnectDb();
			// Debug Helper - console.log('\t - complete');
			process.stdout.write('\n');
		} catch (error) {
			this.disconnectDb();
	...
	async parseKmlToPlacemarks(kml) {
		// Debug Helper - console.log('\t - parsing KML for placemarks');
		process.stdout.write('.');
	...
	shouldSelfReport(lastDBUpdate, lastMTUpdate) {
		...
		if (lastDBUpdate - lastMTUpdate >= minToMSec(gracePeriod)) {
			 ...
			 */
			// Debug Helper - console.log('\t - MarineTraffic is ' + msecToMin(lastDBUpdate - lastMTUpdate) + ' minutes behind Delorme, ' + (msecToMin(lastDBUpdate - lastMTUpdate) - gracePeriod) + ' minutes over the grace period');
			process.stdout.write('>');
			return true;
		}
		...
		if (lastDBUpdate - lastMTUpdate > 0) {
			// Debug Helper - console.log('\t - MarineTraffic is ' + msecToMin(lastDBUpdate - lastMTUpdate) + ' minutes behind Delorme, but ' + (gracePeriod - msecToMin(lastDBUpdate - lastMTUpdate)) + ' minutes remains in grace period');
			process.stdout.write('Δ');
		} else {
			// Debug Helper - console.log('\t - MarineTraffic is up-to-date');
			process.stdout.write('=');
		}
	...
	sendSelfReport(data) {
		...
		// Debug Helper - console.log('\t - Sending update to Marine Traffic.');
		process.stdout.write('>');
		...
		return new Promise(resolve => {
			sendgrid.send(email, (err, json) => {
				...
				// Debug Helper - console.log('\t - e-mail sent successfully.');
